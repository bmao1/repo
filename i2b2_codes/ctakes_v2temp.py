    ...
    with open(infile) as f:
        for line_num, line in enumerate(f):
            ...
            if resource["resourceType"] != "DocumentReference":
                break
            try:
                enctID = "Encounter/" + resource["context"]["encounter"][0]["reference"][-36:]
                if resource["resourceType"] == "DocumentReference":
                    uuid = resource["subject"]["reference"][-36:]
                    encounterdate = resource["date"][:10]
                    # limit time frame to match with classifier for March demo
                    # if encounterdate < "2020-12-30" or encounterdate > "2022-01-02":
                    #    continue
